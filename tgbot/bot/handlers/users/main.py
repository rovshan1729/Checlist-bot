...
from bot.choices import OlimpiadaOrSimulyator, UserLevelChoice


async def user_data_save_type(user, _type):
    if not user._user_data:
        user._user_data = dict()

    user._user_data['type'] = _type
    user.save(update_fields=["_user_data"])
...
async def get_simulyator(message: types.Message, state: FSMContext):
    ...
    olympics = Olimpic.objects.filter(
        is_active=True,
        end_time__gte=timezone.now(),
        type=OlimpiadaOrSimulyator.SIMULYATOR
    ...
    await user_data_save_type(tg_user, "simulyator", )
...
async def get_olympics(message: types.Message, state: FSMContext):
    ...
    olympics = Olimpic.objects.filter(
        is_active=True,
        end_time__gte=timezone.now(),
        type=OlimpiadaOrSimulyator.OLIMPIADA
    ).order_by('start_time', 'end_time')
    ...
    markup = await get_olympics_markup(olympics, language=lang)
    await user_data_save_type(tg_user, "olimpiada")
...
def determine_user_level(average_olympic_score):
    if average_olympic_score < 60:
        level = UserLevelChoice.JUNIOR
    elif 60 < average_olympic_score < 80:
        level = UserLevelChoice.MIDDLE
    elif average_olympic_score >= 80:
        level = UserLevelChoice.SENIOR
    else:
        level = None
...
async def send_next_poll(olympic: Olimpic, user_olimpic: UserOlimpic, user: TelegramProfile):
    user_questions = UserQuestion.objects.filter(
        olimpic=olympic,
        user_olimpic=user_olimpic,
        user=user
    ).values_list("question_id", flat=True)

    olympic_type = user._user_data.get("type", None)

    if olympic_type == "olimpiada":
        average_olympic_score = user.average_olympic_score
        level = determine_user_level(average_olympic_score)

        if level:
            questions = Question.objects.filter(
                olimpic=olympic,
                level=level
            ).exclude(id__in=list(user_questions))
        else:
            questions = Question.objects.filter(
                olimpic=olympic
            ).exclude(id__in=list(user_questions))
    # olimpic_type = sumulyator
    else:
        questions = Question.objects.filter(
            olimpic=olympic,
            level=UserLevelChoice.SUMULATOR
        ).exclude(id__in=list(user_questions))

    if questions.exists() and user_questions.count() < 30:
        ...
        user_question = UserQuestion.objects.create(
            olimpic=olympic, user_olimpic=user_olimpic, user=user,
        ...
        if correct_option:
            ...
            UserQuestionOption.objects.create(
        ...
        olympic_point = olympic.point
        total_olympic_score = user.total_olympic_score
        average_score = user.average_olympic_score
        ...
        total_olympic_score += score
        average_score = total_olympic_score / olympic_point

        user.total_olympic_score = total_olympic_score
        user.average_olympic_score = average_score
        user.save(update_fields=["total_olympic_score", "average_olympic_score"])
        ...
        elif olympic_type == "olimpiada":
            level = determine_user_level(average_score)
            user.user_level = level
            user.save(update_fields=["user_level"])

            result_text = ("🏁 “{olimpic_name}” testi yakunlandi!\n\n"
                           "Natija {result_publish} da e'lon qilinadi\n\n"
                           "Natijalarni '📈 Natijalar 📉' bo'limida ko'rsangiz bo'ladi"
            ).format(
                olimpic_name=user_olimpic.olimpic.title,
                result_publish=result_publish,
            )
        else:
            result_text = "Xatolik yuz berdi"
...
async def start_test(message: types.Message, state: FSMContext):
    ...
    if current_olympic_id:
        olympic = Olimpic.objects.filter(
            id=current_olympic_id
        ).first()
        ...
        if olympic:
            ...
            if start_date_time > now_time:
                await message.answer(_("Simulyator hali boshlanmagan\nBoshlanish sanasi: {start_time}").format(
                    start_time=start_date_time.strftime("%d.%m.%Y")))
                return
            ...
            if olympic.start_time > timezone.now():
                ...
                if questions:
                    await message.answer(
                        _("Test boshlandi, agar savolga javob bera olmasangiz "
                          "savol tagidagi tugma orqali keyigisiga o'tkazib yuborishingiz mumkin ⬇️"),
                    ...
                    user_question = UserQuestion.objects.create(
                        olimpic=olympic, user_olimpic=user_olympic, user=user,
                        question=question, is_sent=True,
                        poll_id=str(poll_message.poll.id),
                        message_id=poll_message.message_id,
                        content_message_id=content_message_id if content_message_id else 0)
...
async def start_test(message: types.Message, state: FSMContext):
    ...
    if current_olympic_id:
        ...
        if olympic:
            ...
            user = get_user(message.from_user.id)
            reset_correct_answers(user)
            user_olympic = UserOlimpic.objects.filter(
                user=user,
                olimpic=olympic
            ).exists()
            ...
            if olympic.start_time > timezone.now():
                ...
                average_score = tg_user.average_olympic_score
                level = determine_user_level(average_score)
                if level:
                    questions = Question.objects.filter(
                        olimpic=olympic,
...
async def skip_test(call: types.CallbackQuery, state: FSMContext):
    ...
    if olympic_id and user_olympic_id:
        last_question = UserQuestion.objects.filter(
            user=user, olimpic_id=olympic_id,
            user_olimpic_id=user_olympic_id
        ).last()
    else:
        last_question = UserQuestion.objects.filter(
            user=user
...
async def finished_test(message: types.Message, state: FSMContext):
    ...
    olimpic_point = last_question.olimpic.point
    user_olimpic = last_question.user_olimpic
    user_questions = UserQuestion.objects.filter(
        olimpic=last_question.olimpic,
        user_olimpic=user_olimpic,
        user=user)
    ...
    score = ((correct / 30) * 100)
    total_olympic_score += score
    average_score = total_olympic_score / olimpic_point

    user.total_olympic_score = total_olympic_score
    user.average_olympic_score = average_score
    user.save(update_fields=[
        "total_olympic_score", "average_olympic_score"
    ])

    not_answered = 30 - answered_count
    result_publish = timezone.template_localtime(user_olimpic.olimpic.result_publish).strftime("%d-%m-%Y %H:%M")
    end_time = timezone.now()
    user_olimpic.end_time = end_time
    user_olimpic.correct_answers = correct
    ...
    if olympic_type == "simulyator":
        ...
        result_text = ("🏁 “{olimpic_name}” testi yakunlandi!\n\n"
                       "Natija {result_publish} da e'lon qilinadi\n\n"
                       "Natijalarni '📈 Natijalar 📉' bo'limida ko'rsangiz bo'ladi").format(
            olimpic_name=user_olimpic.olimpic.title,
            result_publish=result_publish
        )
...
async def get_poll_answer(poll_answer: types.PollAnswer):
    ...
    if user_question:
        ...
        if user_question_option and len(option_ids) == 1:
            ...
            user_question.save(update_fields=[
                "is_answered",
                "is_correct",
                "user_option"
            ])
